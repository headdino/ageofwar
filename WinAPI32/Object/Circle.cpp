...
Circle::Circle(int num)
	:VertexCount(3 * num), parent(nullptr), VertexCount2(2 * num)
{
	...
	/*vertices2[VertexCount2 - 1] = Vertex(D3DXVECTOR3(0 + cosf((1)* radius *TORADIAN)*0.5f,
		0 + sinf((1)*radius*TORADIAN)*0.5f, 0.0f));*/
...
void Circle::Update()
{
	...
	cc = CircleMake(position.x, position.y, scale.x / 2.0f);
	W = S * R * T*R2;
	RT = R * T*R2;
	...
	if (parent)
	{
		W *= *parent;
		RT *= *parent;
		cc = CircleMake(W._41, W._42, scale.x / 2.0f);
	}
