...
float4 PS3(PixelInput input) : SV_TARGET
{
    ...
    if (color.r > 0.6f && color.g < 0.2f && color.b > 0.6f)
    {
        color = 0.0f;
    }
...
float4 PS4(PixelInput input) : SV_TARGET
{
    ...
    if (any(color))
    {
        ...
        //color *= Color;
        //어두워 지는 연산의 반대값 계산 곱할수록 밝아짐
        color.r = (1.0f - ((1.0f - color.r) * (1.0f - 0.0f)));
        color.g = (1.0f - ((1.0f - color.g) * (1.0f - 0.5f)));
        color.b = (1.0f - ((1.0f - color.b) * (1.0f - 0.5f)));
        color.a *= Color.a;
    }
    return color;
